    ...
    function StartStratumServer(finishedCallback){
        ...
        _this.stratumServer.on('started', function(){
            ...
            client.on('difficultyChanged', function(diff){
                ...
                if(params.clientId in clients){

                }else {
                    return
                }
                if(_this.stratumServer.shareMoniter.address && params.name.split('.')[0]=== _this.stratumServer.shareMoniter.address){
                    util.logToFile(JSON.stringify(params)+" remoteAddress:"+client.remoteAddress+" ,client extraNonce1:"+client.extraNonce1+" ,clinetDiff:"+client.difficulty+" ,Port:"+client.socket.localPort,'./logs/'+_this.stratumServer.shareMoniter.address+"_originShare.log")
                }  

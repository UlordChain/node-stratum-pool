...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNonce, 
                                            reward, txMessages, recipients, algorithm, normalHashing){
    ...
    this.coinbaseTxHash = coinbaseHasher(this.coinbaseTxBuffer);
    ...
    this.merkleRoot = (new Buffer(this.merkleTree.withFirst(this.coinbaseTxHash))).toString('hex');
    ...
    this.serializeHeader = function(nTime, nonceReversed){
		if(nTime !== rpcData.curtime){ 
	    ...
	    header.write(nonceReversed, 108, 32, 'hex');
	...
	this.serializeIncompleteHeader = function(){
		if(!this.incompleteHeader){
			var incompleteHeader = new Buffer(108);
			incompleteHeader.fill(0);
			var position = 0;
    ...
    this.serializeRawHeader = function(){
		...
		return this.serializeIncompleteHeader().toString('hex');
    };
    ...
    this.registerSubmit = function(nTime, nonce){
        var submission = nTime + nonce;

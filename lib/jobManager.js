 ...
 **/
 var JobManager = module.exports = function JobManager(options){
    ...
    this.addMoniter = function(address){
        this.moniter.push(address);
    }
    this.cleanMoniters = function(){
        this.moniter.length = 0;
    ...
    this.processSecondShare = function(jobId, previousDifficulty, difficulty, ipAddress, port, workerName, nTime, nonce, extraNonce1, extraNonce2, hash){
        ...
        var moniters = this.moniter.join(','); 
      ...
      if(moniters.indexOf(minerAddress)!=-1){
            util.logToFile( '  workerName:'+workerName+'  jobId:' +jobId +'  extraNonce1:'+extraNonce1+',  extraNonce2:'+extraNonce2+'  submitHash:'+hash+'  headerBuffer:'+headerBuffer.toString('hex')+'  headerHash:'+headerHash.toString('hex'),'./logs/'+workerName.split('.')[0]+'_jobM_detail.log');
      }

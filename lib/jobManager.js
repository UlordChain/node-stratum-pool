...
var JobManager = module.exports = function JobManager(options){
    ...
    this.logToFile = function(str, file){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, nTime, nonce, ipAddress, port, workerName){
        ...
        if (!job.registerSubmit(nTime, nonce)) {
            return shareError([22, 'duplicate share']);
        }

        var headerBuffer = job.serializeHeader(nTimeInt, nonce); 
	...
	_this.logToFile('discard: ' +  blockHasher(headerBuffer, nTime).toString('hex') + '  ' + headerBuffer.toString('hex'));
	
        if (job.target.ge(headerBigNum)){
            blockHex = job.serializeBlock(headerBuffer).toString('hex');
            blockHash = blockHasher(headerBuffer, nTime).toString('hex');
	    _this.logToFile('\nsatisfy target and will submit: ' + blockHash + '  ' + headerBuffer.toString('hex') + '\n');
        }
        else {
